...
vll pre(n , 0);
vll suff(n , 0) ;
for(int i=0;i<n;i++) if(v[i]==1) pre[i]++;
for(int i=0;i<n;i++) if(v[i]==1) suff[i]++;
for(int i=k;i<n;i++) if(v[i-k]==1) pre[i]+=pre[i-k];
for(int i=n-k-1;i>=0;i--)  if(v[i+k]==1)suff[i]+=suff[i+k];
for(int i=0;i<n;i++)
{
    if(prime[v[i]])
    {
